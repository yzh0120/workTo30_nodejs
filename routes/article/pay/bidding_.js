/* 
	 ...
	 */
module.exports = function(req,res,handler,getNowBidding,async){
	async.auto({//asyncRes等于每个属性的callback(null,value)的组成的对象
		...
		select_article : ['check_body_data',function(doing_res,global_callback){
			...
			handler.exec({// 文章id和用户id匹配成功
				res : res,
				callback : global_callback,
			 sql: 'select * from bidding where bidding_article_id = ? and bidding_type = 100',
			 params: [req.body.articleId],
			 success: result => {
				if(result[0]){// 之前已经有竞标记录 更新语句				
					handler.transaction({
						res : res,
						callback : global_callback,
					 sqlArr: ['UPDATE bidding SET bidding_price = bidding_price + ? WHERE bidding_article_id = ? and bidding_type = 100','UPDATE user SET user_gold=user_gold - ? WHERE user_id = ?'],
					 paramsArr: [[req.body.num,req.body.articleId],[req.body.num,req.headers.authorization]],
					 success: result => {
						getNowBidding(req,res,handler,async,'fromBidding')
						global_callback(null,'success')
					 }
					})
				}else{// 之前没有有竞标记录 插入语句
					handler.transaction({
					 ...
					 paramsArr: [[req.body.articleId,req.body.num,100],[req.body.num,req.headers.authorization]],
					 success: result => {
						getNowBidding(req,res,handler,async,'fromBidding')
						global_callback(null,'success')
					 }
