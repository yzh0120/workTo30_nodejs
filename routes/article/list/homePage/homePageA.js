...
module.exports = function(req,res,fs,path,handler,async,mysqlG,globalVar){
	async.auto({//asyncRes等于每个属性的callback(null,value)的组成的对象
		...
		select_article : ['check_body_data',function(doing_res,global_callback){
			...
			handler.exec({//在数据库中查询数据
				res : res,
				callback : global_callback,
			    sql: `sELECT article.*,${mysqlG.userBase} FROM article , user where article.article_user_id = user.user_id ${time_sql} ORDER BY article.${value} desc LIMIT ${globalVar.limit}`,
			    params: [],
			    success:  result => {
						/* ******************************************** 先定义需要异步的函数*/
