...
module.exports = function(req,res,handler,async,fromBidding){
	async.auto({//asyncRes等于每个属性的callback(null,value)的组成的对象
		...
		select_article : ['check_body_data',function(doing_res,global_callback){
			handler.exec({//查询bidding_type = 100 limit5的竞标结果
				res : res,
				callback : global_callback,
			    sql: 'select * from bidding where bidding_type = 100 ORDER BY bidding_price desc LIMIT 5',
			    params: [],
			    success:  result => {
						/* ******************************************** 先定义需要异步的函数*/
						...
						,function(error, asyncRes){
							...
							/* ************************************************* */
							if(req.body.articleId){/* 如果有articleId则是在进入竞标页面,fromBidding是指点击竞标后的计算*/
								handler.exec({//根据文章查竞标的价值
								res : res,
								callback : global_callback,
								 sql: 'select bidding_price from bidding where bidding_article_id = ? and bidding_type = 100',
